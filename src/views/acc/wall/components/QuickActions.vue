...
async function confirm(){
  ...
  // await defHttp.post({ url:'/api/acc/control/execute', params:{ action: action.value, ...vals } });
  visible.value = false;
...
const { tableContext: logCtx } = useListPage({
  ...
  tableProps:{
    ...
    api:(p:any)=>defHttp.get({ url:'/api/acc/control/log', params:{...p} }),
  }
});
...
.log-title{ margin-top:12px; margin-bottom:4px; font-weight:600; }
</style>
