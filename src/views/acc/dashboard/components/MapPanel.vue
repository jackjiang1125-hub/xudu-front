...
function groupByState(list: GatePoint[]){
  const grp: Record<string, GatePoint[]> = { OPEN:[], CLOSE:[], LOCKED:[], ALARM:[], DURESS:[] } as any;
...
function render(){
  ...
  chart.setOption({
    tooltip: { trigger:'item', textStyle:{ color:'#0b0f1a' } },
...
async function load(){
  try{
    ...
    points = USE_MOCK ? fallbackMock() : fallbackMock();
  }catch(e){
    points = fallbackMock();
  }
  render();
...
<style scoped>
.panel-title{ font-weight:600; margin-bottom:8px; letter-spacing:.5px; display:flex; align-items:center; gap:12px; color:#f8fbff; }
.legend{ font-size:12px; opacity:.9; display:flex; gap:10px; align-items:center; color:#f8fbff; }
